...
const UpsertUser = (call, callback) => {
  ...
  // Nếu có ID, cập nhật user
  if (id) {
    const userIndex = db.users.findIndex((user) => user.id === id);
    if (userIndex === -1) {
      return callback(null, { success: false, message: "User không tồn tại" });
    }
  ...
  const newUser = { id: uuidv4(), name, email, password, avatar };
  db.users.push(newUser);
  writeDb(db);
  callback(null, { success: true, message: "Thêm user thành công" });
};
...
const DeleteFolder = (call, callback) => {
  const { id } = call.request;
  const db = readDb();
  db.notes = db.notes.filter((folder) => folder.id !== id);
  writeDb(db);
...
const AddNote = (call, callback) => {
  const { folderId, content } = call.request;
  const db = readDb();
  const note = { id: uuidv4(), folderId, content };
  db.notes.push(note);
  writeDb(db);
